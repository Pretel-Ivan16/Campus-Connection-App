...
import type { Faculty, UpdateFacultyRequest } from '../../services/faculty.service';
...
type EditFacultyForm = z.infer<typeof editFacultySchema>;

interface EditFacultyModalProps {
  isOpen: boolean;
  faculty: Faculty | null;
  onClose: () => void;
  onSubmit: (data: UpdateFacultyRequest) => Promise<void>;
  isLoading?: boolean;
}

export const EditFacultyModal = ({ isOpen, faculty, onClose, onSubmit, isLoading }: EditFacultyModalProps) => {
  ...
  // Actualizar valores cuando cambia la facultad
  useEffect(() => {
    if (faculty) {
      reset({
        name: faculty.name,
  ...
  const handleFormSubmit = async (data: EditFacultyForm) => {
    try {
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-[#1a1d2e] border border-[#2a2a2a] rounded-lg p-8 max-w-md w-full shadow-xl">
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-white">Editar Facultad</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4" translate="no">
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              type="submit"
              disabled={isSubmitting || isLoading}
              className="flex-1 px-4 py-2 bg-[#6483ff] hover:bg-[#5474e0] text-white rounded-lg font-semibold transition-colors disabled:opacity-50"
            >
