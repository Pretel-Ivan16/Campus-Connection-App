...
if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
  ...
  const rawApiUrl = import.meta.env.VITE_API_URL || 'https://campus-connection-app-4.onrender.com/api';
  const normalizedApiUrl = rawApiUrl.replace(/\/$/, '');
  API_BASE_URL = normalizedApiUrl.endsWith('/api') ? normalizedApiUrl : `${normalizedApiUrl}/api`;
}
